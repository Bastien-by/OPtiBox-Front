...
interface Stock {
  id: number;
  alitracer: string;  // peut être "ALI001" ou "ALI001|ALI002"
  lockerNumber: number;
  available: boolean;
  status: number;
...
export class DepositpageComponent implements OnInit, OnDestroy {
  ...
  stopDepositScan(): void {
    ...
    if (this.scannedTools.length === 0) {
      this.messageService.add({ severity: 'warn', summary: 'Aucun outil scanné', detail: "Vous n'avez scanné aucun outil à déposer." });
      return;
    }
    this.startCheckFlow();
  }
  ...
  private async handleScannedCode(decodedText: string): Promise<void> {
    try {
      ...
      // Vérifie doublon (compare sur l'alitracer scanné contre tous les alitracers du stock)
      const alreadyScanned = this.scannedTools.some(t => StockService.matchesAlitracer(t, alitracer));
      if (alreadyScanned) {
        this.messageService.add({ severity: 'warn', summary: 'Déjà scanné', detail: `L'outil ${alitracer} a déjà été scanné.` });
        return;
      }
      ...
      if (!stock) {
        this.messageService.add({ severity: 'error', summary: 'Outil inconnu', detail: `L'alitracer ${alitracer} ne correspond à aucun outil.` });
        return;
      }
      if (stock.available) {
        this.messageService.add({ severity: 'warn', summary: 'Outil déjà disponible', detail: `L'outil ${alitracer} est déjà dans son casier.` });
        return;
      }
      if (stock.status === 2) {
        this.messageService.add({ severity: 'error', summary: 'Outil HS', detail: `L'outil ${alitracer} est HS et ne peut pas être déposé.` });
        return;
      }

      this.scannedTools.push(stock);
      this.messageService.add({ severity: 'success', summary: 'Outil ajouté', detail: `${alitracer} ajouté (Casier ${stock.lockerNumber})` });
    } catch {
      this.messageService.add({ severity: 'error', summary: 'Erreur', detail: 'Une erreur est survenue lors du traitement du scan.' });
    } finally {
  ...
  private openNextToolForCheck(): void {
    const next = this.scannedTools.find(t => t._checked !== true as any);
    if (!next) {
  ...
  /**
   * Retourne les alitracers d'un stock sous forme de tableau pour l'affichage.
   */
  ...
  async confirmCheck(status: number): Promise<void> {
    ...
    if (!appUser) { this.messageService.add({ severity: 'error', summary: 'Erreur', detail: 'Vous devez être connecté.' }); return; }

    const checkDate = new Date().toISOString();
    const payload: any = {
      id: null, date: checkDate, status, comment: this.checkComment || '',
      checkType: 'INDIVIDUAL',   // individuel — n'impacte pas le ratio ni l'Excel
    ...
    try {
      ...
      this.messageService.add({ severity: 'success', summary: 'Contrôle enregistré', detail: `Statut ${this.getStatusLabelFromStock(this.selectedStockForCheck)} enregistré pour ${this.selectedStockForCheck.alitracer}.` });
    } catch { this.messageService.add({ severity: 'error', summary: 'Erreur', detail: "Erreur lors de l'enregistrement du contrôle." }); }
  ...
  private groupToolsByLocker(): void {
    ...
    for (const tool of this.scannedTools) {
      if (!grouped.has(tool.lockerNumber)) grouped.set(tool.lockerNumber, []);
      grouped.get(tool.lockerNumber)!.push(tool);
    }
    this.lockerQueue = Array.from(grouped.entries()).map(([locker, tools]) => ({ locker, tools })).sort((a, b) => a.locker - b.locker);
  }

  private async processNextLocker(): Promise<void> {
    if (this.currentLockerIndex >= this.lockerQueue.length) {
      this.messageService.add({ severity: 'success', summary: 'Dépôt terminé', detail: 'Tous les outils ont été contrôlés et déposés.' });
    ...
    try {
      ...
      this.messageService.add({ severity: 'success', summary: 'Casier ouvert', detail: `Casier ${this.currentLocker} ouvert. Rangez les outils.` });
    } catch {
      this.messageService.add({ severity: 'error', summary: 'Erreur', detail: `Impossible d'ouvrir le casier ${this.currentLocker}.` });
  ...
  async closeCurrentLocker(): Promise<void> {
    ...
    try {
      ...
      this.messageService.add({ severity: 'success', summary: 'Casier fermé', detail: `Casier ${this.currentLocker} fermé avec succès.` });
      await this.updateStocksAndHistory();
      this.currentLockerIndex++;
      await new Promise(resolve => setTimeout(resolve, 500));
      await this.processNextLocker();
    } catch {
      this.messageService.add({ severity: 'error', summary: 'Erreur', detail: `Impossible de fermer le casier ${this.currentLocker}.` });
    }
  }
  ...
  private async updateStocksAndHistory(): Promise<void> {
    ...
    for (const tool of this.currentLockerTools) {
      try {
        await this.scanService.createHistory({ stock: { id: tool.id }, user: { id: appUser.id }, date: new Date().toISOString(), type: 'deposit' });
      } catch { this.messageService.add({ severity: 'error', summary: 'Erreur dépôt', detail: `Impossible d'enregistrer le dépôt de ${tool.alitracer}.` }); }
    }
